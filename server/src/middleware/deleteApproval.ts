...
export const processDeleteRequest = async (req: Request, res: Response, next: NextFunction, originalFn: any) => {
  try {
    ...
    const reason = req.headers['x-delete-reason'] as string || req.body?.reason;
    if (!reason) {
    ...
    const managerRoles = ['hr_admin', 'operations_admin', 'finance_admin', 'finance_sub_admin', 'sales_manager', 'superadmin'];
