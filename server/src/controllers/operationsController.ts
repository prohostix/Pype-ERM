...
export const updateProgram = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (registrationFee !== undefined || tuitionFee !== undefined || installments !== undefined) {
    ...
    if (existingFee) {
      const feeData: any = {};
      if (registrationFee !== undefined) feeData.registrationFee = Number(registrationFee);
      if (tuitionFee !== undefined) feeData.tuitionFee = Number(tuitionFee);
      if (billingCycle !== undefined) feeData.billingCycle = billingCycle;
      if (installments !== undefined) feeData.installments = installments;
