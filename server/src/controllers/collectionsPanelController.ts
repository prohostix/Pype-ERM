...
export const getCollectionOverseers = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  res.status(200).json({
    success: true,
    data: overseers.map((o: any) => o.user)
  });
});

// POST /collections/overseers
export const addCollectionOverseer = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const removeCollectionOverseer = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const getCollectionMetrics = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const studentStatusCounts = students.reduce((acc: any, s: any) => {
  ...
  const todayCollectedSchedules: any[] = [];
  const todayDueSchedules: any[] = [];
  const upcomingSchedules: any[] = [];
  const overdueSchedules: any[] = [];
  const pendingSchedules: any[] = [];
  ...
  res.status(200).json({
    ...
    data: {
      ...
      currentUserOversight: {
        isAdmin: ['org_admin', 'superadmin', 'finance_admin'].includes(req.user.role),
