...
export const getPunchConfig = asyncHandler(async (req: AuthRequest, res: Response) => {
  if (!req.user.organizationId) {
...
export const syncOfflinePunches = asyncHandler(async (req: AuthRequest, res: Response) => {
  if (!req.user.organizationId) {
    res.status(400).json({ success: false, message: 'Only available for tenant organization employees.' });
    return;
  }
