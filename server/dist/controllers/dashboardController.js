...
export const getDashboardMetrics = asyncHandler(async (req, res) => {
    ...
    const metrics = {};
    const orgQuery = role === 'superadmin' ? {} : { organizationId: orgId };
    if (role === 'superadmin') {
    ...
    if (['ops_admin', 'ops_sub_admin', 'ceo', 'org_admin'].includes(role)) {
        ...
        metrics.documentsPending = studentsStatus.filter(s => {
            ...
            const docs = Array.isArray(s.documents) ? s.documents : [];
            const unapprovedDocs = docs.filter((d) => d && d.status !== 'approved');
            const photoStatus = s.admissionProgress?.photoStatus;
            return {
                id: s.id,
                name: s.name,
...
export const getFinanceOverviewMetrics = asyncHandler(async (req, res) => {
    const orgId = req.user.organizationId;
    const { dateRange } = req.query;
    const branchId = req.query.branchId;
    const whereBase = { organizationId: orgId };
    ...
    const totalInflow = allPayments.reduce((sum, p) => sum + p.amount, 0) +
    ...
    const totalOutflow = allExpenses.reduce((sum, e) => sum + e.amount, 0) +
