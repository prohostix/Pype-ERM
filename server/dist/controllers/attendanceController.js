...
export const punchIn = asyncHandler(async (req, res) => {
    ...
    await broadcastNotification(req.user.organizationId, 'general', 'Employee Punched In', `${req.user.name} just punched in ${isLate ? `(Late by ${lateMinutes} mins)` : '(On Time)'}.`, ['hr_admin', 'hr_sub_admin']);
    res.status(201).json({
        success: true,
...
export const punchOut = asyncHandler(async (req, res) => {
    ...
    await broadcastNotification(req.user.organizationId, 'general', 'Employee Punched Out', `${req.user.name} just punched out after ${workingHours} hrs.`, ['hr_admin', 'hr_sub_admin']);
    res.json({
        success: true,
...
export const getAttendances = asyncHandler(async (req, res) => {
    ...
    const userFilters = { role: { not: 'student' } };
    ...
    if (isDateFiltered && (!requestedStatus || mockStatuses.includes(requestedStatus))) {
        ...
        if (where.user) {
            ...
            if (where.user.studyCenterId)
                orgQuery.studyCenterId = where.user.studyCenterId;
            if (where.user.departmentId)
                orgQuery.departmentId = where.user.departmentId;
            if (where.user.OR)
                orgQuery.OR = where.user.OR;
        }
