...
export const validateStudentInviteToken = asyncHandler(async (req, res) => {
...
export const getSalesEnrollmentPipeline = asyncHandler(async (req, res) => {
...
export const approveSalesEnrollmentOps = asyncHandler(async (req, res) => {
    const now = new Date();
    ...
    const history = enrollment.statusHistory || [];
    history.push({
...
export const approveSalesEnrollmentFinance = asyncHandler(async (req, res) => {
    const now = new Date();
    const enrollment = await prisma.enrollment.findUnique({ where: { id: req.params.id } });
    if (!enrollment || enrollment.organizationId !== req.user.organizationId) {
        res.status(404).json({ success: false, message: 'Enrollment not found' });
...
// ─── Ops/Finance: Reject ──────────────────────────────────────────────────────
export const rejectSalesEnrollment = asyncHandler(async (req, res) => {
...
export const createDirectEnrollment = asyncHandler(async (req, res) => {
...
export const verifySalesEnrollment = asyncHandler(async (req, res) => {
    ...
    if (updatedEnrollment.studentId) {
        ...
        const progress = existingStudent?.admissionProgress || {};
