...
export const createPayment = asyncHandler(async (req, res) => {
    ...
    if (!invoiceId && req.body.studentId) {
        ...
        let targetScheduleId = undefined;
        if (scheduleId) {
...
export const createFeeStructure = asyncHandler(async (req, res) => {
    const { programId, universityId, feeLevel, sessionId, registrationFee, tuitionFee, examFee, universityFee, commissionRate, yearlyFees, gstPercentage, billingCycle, currency, effectiveFrom, dueDate, additionalFees, allowInitialFee, specialisation, installments } = req.body;
    const data = {
        organizationId: req.user.organizationId,
...
// Auth Fees
export const getAuthFees = asyncHandler(async (req, res) => {
    ...
    const mapped = fees.map(f => ({
        ...
        amount: f.feeDetails?.amount ?? 0,
        currency: f.feeDetails?.currency ?? 'INR',
    }));
    res.json({ success: true, data: mapped });
});
export const createAuthFee = asyncHandler(async (req, res) => {
    const { universityId, amount, currency } = req.body;
    if (!universityId || !amount)
        return res.status(400).json({ success: false, message: 'universityId and amount are required' });
...
export const updateAuthFee = asyncHandler(async (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ success: false, message: 'Auth fee not found' });
    const prevDetails = existing.feeDetails;
...
// Centers
export const getPendingPaymentCenters = asyncHandler(async (req, res) => {
...
export const financeVerifyCenter = asyncHandler(async (req, res) => {
...
export const getUniversityCommissions = asyncHandler(async (req, res) => {
    const organizationId = req.user.organizationId;
...
export const recordUniversityCommission = asyncHandler(async (req, res) => {
    const organizationId = req.user.organizationId;
...
export const getUniversityPayments = asyncHandler(async (req, res) => {
    const organizationId = req.user.organizationId;
    const students = await prisma.student.findMany({
        where: {
            organizationId,
...
export const recordUniversityPayment = asyncHandler(async (req, res) => {
    const organizationId = req.user.organizationId;
