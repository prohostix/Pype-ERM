...
export const getUniversityReviewQueue = asyncHandler(async (req, res) => {
...
export const approveUniversityEnrollment = asyncHandler(async (req, res) => {
    const universityId = req.user.universityId;
    const now = new Date();
    if (!universityId) {
        res.status(403).json({ success: false, message: 'No university linked to your account' });
    ...
    if (enrollment.program?.universityId !== universityId) {
    ...
    const updatedStatusHistory = enrollment.statusHistory || [];
        ...
        for (const u of recipients) {
            await prisma.notification.create({
                data: {
                    ...
                    message: `${enrollment.studentName}'s enrollment in ${enrollment.program?.name} has been approved by the university. The student is now officially enrolled.`,
                    type: 'general',
                    priority: 'medium',
...
export const rejectUniversityEnrollment = asyncHandler(async (req, res) => {
        ...
        for (const u of recipients) {
            await prisma.notification.create({
                data: {
                    ...
                    message: `${enrollment.studentName}'s enrollment in ${enrollment.program?.name} was rejected by the university. Reason: ${remarks}`,
                    type: 'general',
                    priority: 'high',
                    link: 'enrollment/enrollments',
                },
...
//# sourceMappingURL=universityEnrollmentController.js.map