...
export const createGSTSetting = asyncHandler(async (req, res) => {
...
export const getGSTSetting = asyncHandler(async (req, res) => {
...
export const updateGSTSetting = asyncHandler(async (req, res) => {
    ...
    if (sacCode !== undefined)
        updateData.sacCode = sacCode;
    if (applicableFrom !== undefined)
        updateData.applicableFrom = new Date(applicableFrom);
    if (applicableTo !== undefined)
        updateData.applicableTo = applicableTo ? new Date(applicableTo) : null;
    if (allowOverride !== undefined)
        updateData.allowOverride = allowOverride;
    if (status !== undefined)
        updateData.status = status;
    const setting = await prisma.gSTSetting.update({ where: { id: req.params.id }, data: updateData });
    res.json({ success: true, data: setting });
});
export const deleteGSTSetting = asyncHandler(async (req, res) => {
    const exists = await prisma.gSTSetting.findFirst({ where: { id: req.params.id, organizationId: req.user.organizationId } });
    if (!exists) {
        res.status(404).json({ success: false, message: 'GST setting not found' });
        return;
...
export const getApplicableGST = asyncHandler(async (req, res) => {
    const setting = await prisma.gSTSetting.findFirst({
