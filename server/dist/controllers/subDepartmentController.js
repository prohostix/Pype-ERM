...
export const createSubDepartment = asyncHandler(async (req, res) => {
    const { name, parentDeptId, features, assignedUniversities, assignedPrograms, assignedCenters } = req.body;
    ...
    const subDepartment = await prisma.subDepartment.create({
        data: {
            ...
            features: features || [],
            // Handle relations
            assignedUniversities: assignedUniversities ? { connect: assignedUniversities.map((id) => ({ id })) } : undefined,
            assignedPrograms: assignedPrograms ? { connect: assignedPrograms.map((id) => ({ id })) } : undefined,
            assignedCenters: assignedCenters ? { connect: assignedCenters.map((id) => ({ id })) } : undefined,
        },
    });
    res.status(201).json({
        success: true,
...
// @access  Private
export const getSubDepartments = asyncHandler(async (req, res) => {
...
// @access  Private
export const getSubDepartment = asyncHandler(async (req, res) => {
...
export const updateSubDepartment = asyncHandler(async (req, res) => {
    ...
    if (managerId !== undefined)
        data.managerId = (managerId && managerId !== '') ? managerId : null;
    // Handle relation updates (set/disconnect/connect)
    if (assignedUniversities !== undefined)
        data.assignedUniversities = { set: assignedUniversities.map((id) => ({ id })) };
    if (assignedPrograms !== undefined)
        data.assignedPrograms = { set: assignedPrograms.map((id) => ({ id })) };
    if (assignedCenters !== undefined)
        data.assignedCenters = { set: assignedCenters.map((id) => ({ id })) };
...
export const getMySubDepartment = asyncHandler(async (req, res) => {
