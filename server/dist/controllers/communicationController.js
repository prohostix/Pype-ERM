...
const logCommunication = async (organizationId, userId, type, recipientCount, content) => {
...
export const sendBulkEmail = asyncHandler(async (req, res) => {
    ...
    const log = await logCommunication(req.user.organizationId, req.user.id, 'email', recipientCount || 0, JSON.stringify({ subject, message, filters }));
...
export const sendBulkSms = asyncHandler(async (req, res) => {
    ...
    const log = await logCommunication(req.user.organizationId, req.user.id, 'sms', recipientCount || 0, JSON.stringify({ message, filters }));
    res.status(200).json({ success: true, data: log, message: 'SMS campaign started successfully' });
});
export const sendBulkWhatsApp = asyncHandler(async (req, res) => {
    const { message, recipientCount, filters } = req.body;
    // TODO: In the future, integrate with WhatsApp Business API here
    const log = await logCommunication(req.user.organizationId, req.user.id, 'whatsapp', recipientCount || 0, JSON.stringify({ message, filters }));
