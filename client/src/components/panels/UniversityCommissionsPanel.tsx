...
interface StudentWithCommissions {
  id: string;
  name: string;
  email: string;
  phone: string | null;
...
export function UniversityCommissionsPanel() {
  const [students, setStudents] = useState<StudentWithCommissions[]>([]);
  const [loading, setLoading] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState<StudentWithCommissions | null>(null);
  ...
  const handleRecordClick = (student: StudentWithCommissions) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/finance/commissions', payload);
      toast.success('University commission payment recorded successfully');
      setDialogOpen(false);
      fetchCommissions();
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to record commission');
    }
  };
    ...
    if (feeStructure) {
      if (feeStructure.billingCycle === 'per_year' && Array.isArray(feeStructure.yearlyFees) && feeStructure.yearlyFees.length > 0) {
        feeStructure.yearlyFees.forEach((yf: any) => {
          ...
          expectedCommission += (uFee * cRate) / 100;
        });
        if (feeStructure.yearlyFees.length > 0) {
          baseCommissionRate = Number(feeStructure.yearlyFees[0].commissionRate || 0);
        }
      } else {
        totalUniversityFee = feeStructure.universityFee || 0;
        baseCommissionRate = feeStructure.commissionRate || 0;
        expectedCommission = (totalUniversityFee * baseCommissionRate) / 100;
      }
    }

    const received = student.commissions.reduce((sum, c) => sum + (c.amountReceived || 0), 0);
    const balance = Math.max(0, expectedCommission - received);
    
    let statusLabel = 'Unpaid';
    let statusColor = 'bg-rose-500/10 text-rose-500 border-rose-500/20';

    if (received >= expectedCommission && expectedCommission > 0) {
  ...
  const getOverallStats = () => {
    ...
    let totalReceived = 0;
    students.forEach(s => {
      const { expectedCommission, received } = calculateCommissionData(s);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">University Commission Management</h2>
        <p className="text-muted-foreground">Track and log commissions received from universities against student admissions</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-none shadow-sm bg-card/60 backdrop-blur">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground uppercase font-semibold">Total Expected Commission</p>
        ...
        <Card className="border-none shadow-sm bg-card/60 backdrop-blur">
          <CardContent className="p-5 flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground uppercase font-semibold">Total Received Commission</p>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th className="p-3 font-semibold text-muted-foreground text-right">Expected Commission</th>
                ...
                <tbody>
                  {students
                    ...
                    .map((student) => {
                      ...
                      statusLabel,
                      statusColor
                    } = calculateCommissionData(student);
                    ...
                    return (
                      <tr key={student.id} className="border-b border-border hover:bg-muted/10 transition-colors">
                        ...
                        <td className="p-3 text-right font-bold text-primary">₹{expectedCommission.toLocaleString('en-IN')}</td>
                        ...
                        <td className="p-3 text-center">
                          <Button
                            ...
                            disabled={balance === 0 && expectedCommission > 0}
                          >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Log Received Commission</DialogTitle>
          </DialogHeader>
          {selectedStudent && (() => {
            const { expectedCommission, received, balance } = calculateCommissionData(selectedStudent);
            return (
              <form onSubmit={handleSubmit} className="space-y-4 pt-2">
                <div className="p-3 rounded-lg bg-muted/40 space-y-1">
                  <p className="text-xs text-muted-foreground">Expected Commission: <span className="font-bold text-foreground">₹{expectedCommission.toLocaleString('en-IN')}</span></p>
                  <p className="text-xs text-muted-foreground">Already Received: <span className="font-bold text-emerald-500">₹{received.toLocaleString('en-IN')}</span></p>
                ...
                <div className="flex gap-2 border-t pt-4">
                  <Button type="submit" className="flex-1">Submit Payout</Button>
